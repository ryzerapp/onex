...
## What's implemented (2026-02-22 — Framer→Brevo silent-drop fix)
- **Root cause**: Cloudflare ingress was injecting `Access-Control-Allow-Origin: *` while our middleware also returned `Allow-Credentials: true` — a browser-rejected combination. Framer Code Override fetches were preflighted, the response was spec-violating, and the actual POST body never reached our pod. Silent drop = no Brevo contact, no admin email.
- **Server-side**:
  - `app.add_middleware(CORSMiddleware, allow_origin_regex=".*")` (echoes the actual Origin so the credentials combination is valid). The Cloudflare wildcard override still surfaces externally, but our endpoint now also accepts CORS-immune transports.
  - `/api/waitlist/join` rewritten to manually parse the body so it accepts BOTH `application/json` AND `text/plain` (which is the only content-type browsers allow for `mode:"no-cors"` and `navigator.sendBeacon`).
  - New inbound logging: `waitlist/join inbound origin=… referer=… source=… ref=… email=…` so every hit is loud, not silent.
  - New `GET /api/waitlist/diag` — public connectivity ping returning `{ok, signups_total, brevo_list_id, now}` so users can verify from their Framer site's DevTools Console.
- **Code Override v2** (bulletproof): now fires via **`navigator.sendBeacon` first**, then a **`mode:"no-cors"` fetch** as a belt-and-suspenders backup. Neither path triggers a CORS preflight; both reach our server regardless of Cloudflare's wildcard behaviour. The override also does an initial form scan + MutationObserver-based subtree watch so it works whether Framer mounts the `<form>` synchronously or async.
- **Invite & Earn UI**: install instructions extended with a "Verify it works from your Framer site" section — a one-liner the user pastes into Framer DevTools to confirm the API endpoint is reachable and to see the live `signups_total` counter increment per submission.

## What's implemented (2026-02-22 — 403 email-CTA fix + login pre-fill)
- **Fixed the 403 on email CTAs.** Every outbound email used to compute its CTA host from `request.base_url`, which inside the Kubernetes cluster resolves to `*.cluster-12.preview.emergentcf.cloud` and returns 403 to the browser. Replaced 11 occurrences in `server.py` with a single new helper `_app_url(request)` that always returns `ONEX_APP_URL` from env (with a sanitised Origin-header fallback that explicitly rejects internal cluster hosts).
- New env var **`ONEX_APP_URL=https://exclusive-members-1.preview.emergentagent.com`** is the canonical public host for app-side links; `PUBLIC_APP_URL=https://www.onexassets.com` remains the marketing/Framer host for referral links.
- **Waitlist welcome email now deep-links into `/login?email=…&ref=…&src=waitlist`** with the visitor's email pre-filled — so clicking the CTA drops them straight into the email-OTP entry box (or one tap to "Use Google instead"). No re-verification; the referral code still survives via sessionStorage.
- **Login page** now reads `?email=` + `?ref=` from the URL on mount and auto-switches to `email-input` mode with the email pre-populated.
